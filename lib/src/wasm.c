...
TSWasmLanguage *ts_wasm_store_load_language(
  ...
  uint32_t language_name_len,
  const char *wasm,
  uint32_t wasm_len
) {
  // Build language function name string.
  unsigned prefix_len = strlen("tree_sitter_");
  char language_function_name[prefix_len + language_name_len + 1];
  memcpy(&language_function_name[0], "tree_sitter_", prefix_len);
  memcpy(&language_function_name[prefix_len], language_name, language_name_len);
  language_function_name[prefix_len + language_name_len] = '\0';
  ...
  for (unsigned i = 0; i < exports.size; i++) {
    ...
    if (name_eq(name, "__wasm_apply_data_relocs")) {
      ...
      trap = wasm_func_call(apply_relocation_func, &arguments, &results);
      if (trap) {
        wasm_message_t message;
        wasm_trap_message(trap, &message);
  ...
  memcpy(&wasm_language, &memory[language_address_val.of.i32], sizeof(LanguageInWasmMemory));
  ...
  *result = (TSWasmLanguage) {
  ...
  LanguageInstance language_instance = {
    .id = result->id,
  ...
  array_push(&self->language_instances, language_instance);
  return result;
}
